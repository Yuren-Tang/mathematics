...
- exact authorial mathematical checkpoint: `9dc0b3a5c906e51f8f1816e00b85f7aa2a744b1b`;
...
## 2. Safe-boundary and source audit

The target branch was absent before intake and was created exactly from `ec765cd...`.

The PDL mathematical checkpoint diverges from the unified base and contains unrelated B2/B8 source-fidelity material. The candidate consumes only:

1. the four OR1 dossiers at `9dc0b3a...`;
2. the OR1 DAG registration at `29657f...`, as dependency metadata only;
3. the finalized handoff at `9cb286...`, as the explicitly requested packaging/control surface.

During final immutability checking, the live PDL branch was found to be exactly two commits beyond the mathematical checkpoint: the named DAG registration and the finalized handoff. This was the next safe intake boundary and introduced no new mathematics. The handoff was therefore read and consumed before final return. Later PDL commits, if any, remain excluded.

No complete PDL tree or unrelated proof-development dossier is merged or copied.

Before this final audit update, the candidate was:

- `21` commits ahead of the exact base;
- `0` behind;
- merge base exactly `ec765cd03271abd3588ec36faec3d53d0f8aa03b`;
- `17` changed paths.

This update is the twenty-second candidate commit and changes only this final audit path.
...
No `proof-development/`, `complete-cdc/`, `five-support/`, `reduction/`, Lean, manuscript, release, workflow, tag, or DOI path is modified on the Curator branch.

## 4. Handoff success-test audit

The finalized PDL handoff requires eight visible distinctions. The candidate retains all eight:
...
4. per-lift nonautomaticity versus open fixed-fibre/global existence;
5. retained face occurrences versus generic support decomposition;
6. support-only data loss versus enriched oriented collapse;
7. loopless theorem scope versus explicit opposite loop-dart reinsertion;
8. authorial proof, independent audit, and Lean status as separate axes.

The handoff's recommended destinations are all represented: a distinct orientation chapter group, theorem DAG, assurance/formal-status surfaces, frontier map, and outer-shell distinction.

## 5. Retained-data surface reconstruction
...
## 6. Fixed-lift obstruction audit
...
## 8. TeX/control-character repair
...
The $K_4$ result is retained as an exact theorem/counterexample, not census evidence.

## 11. Outer-shell audit
...
No PDL proof-expansion defect or additional Research Lead obligation was found.

## 13. Assurance audit

- OR1 mathematical proofs: authorial PDL complete-draft, Curator-integrated;
- finalized handoff: consumed at exact packaging commit, no theorem upgrade;
...
Only `Yuren-Tang/mathematics:curation/affine-cdc-orientation-obstruction-v1` is written by the Curator.

Unchanged by Curator action:

- the exact unified base branch and commit;
- `mathematics:main`;
- the exact PDL mathematical checkpoint, DAG registration, and handoff commits;
...
The PDL branch's two OR1 packaging commits were produced by AC-PDL and only read here. No force-push, rebase, squash, merge, PR, branch deletion, rename, source rewrite, or canonical movement occurs.
...
- the four dossiers, DAG registration, and finalized handoff are mutually consistent;
